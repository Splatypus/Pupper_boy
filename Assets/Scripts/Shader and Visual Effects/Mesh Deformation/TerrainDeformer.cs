...
public class TerrainDeformer : MonoBehaviour {
	...
	void Start () {
        ...
        initialHeights = terrain.terrainData.GetHeights(0, 0, terrain.terrainData.heightmapResolution, terrain.terrainData.heightmapResolution);
	}
    ...
    void Update () {
        ...
        if (Input.GetKey(KeyCode.C)) {
            int x = (int)((transform.position.x - terrainObject.transform.position.x) / terrainObjectWidth * (float)terrain.terrainData.heightmapResolution); //the player's x position on the terrain heightmap
            int y = (int)((transform.position.z - terrainObject.transform.position.z) / terrainObjectHeight * (float)terrain.terrainData.heightmapResolution); //and their y position (or is it z? Thanks for making y up....)
