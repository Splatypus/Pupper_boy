...
Properties { 
	...
	_WaveHeight ("Wave Height", Float) = 1.3
...
CGINCLUDE 
	...
	v2f vert(appdata_full v)
	{
		...
		o.viewInterpolator.xyz = v.vertex - _WorldSpaceCameraPos;//worldSpaceVertex - _WorldSpaceCameraPos;
		...
		o.normalDir = v.normal;//normalize( mul(float4(v.normal, 0.0), modelMatrixInverse).xyz); Find the tangent plane to the equation that defines the y value at point (x,z). The normal of that plane is what we need here.
	...
	{ 
		...
		half4 foam = Foam(_ShoreTex, i.bumpCoords * 4.0);
